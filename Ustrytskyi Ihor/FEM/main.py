...
class ExperimentConfig:
    def __init__(self,
                 ...
                 use_neural_refinement=False,
                 neural_model_path=None,
                 collect_dataset=False,
                 dataset_output_path="dataset.h5"):
        self.r_min = r_min
        self.r_max = r_max
        self.z_min = z_min
        self.z_max = z_max
        ...
        # Neural refinement
        self.use_neural_refinement = use_neural_refinement
...
def adaptive_refine_and_solve(
    ...
    # Neural refinement predictor (loaded once if enabled)
    _neural_predictor = None
    if getattr(config, 'use_neural_refinement', False) and getattr(config, 'neural_model_path', None):
        from .neural_refinement.predictor import NeuralRefinementPredictor
        _neural_predictor = NeuralRefinementPredictor(config.neural_model_path)

    # Dataset collection state
    _dataset_rows: list = []
    _pending_gp_features: list = []  # GP features waiting for refined targets
    ...
    for cycle in range(config.adaptive_max_cycles):
        ...
        # --- Dataset collection: record coarse GP features ---
        if getattr(config, 'collect_dataset', False) and config.adaptive_max_cycles != cycle + 1:
            eta_all = ibem_strategy.last_eta_all
            gp_data = ibem_strategy.last_gp_data
            p_val = getattr(config, 'p', 0.0)
            nu_val = getattr(config, 'nu', 0.3)
            mu_val = getattr(config, 'mu', 1.0)
            r_min_v = min(n.r for n in mesh.nodes.values())
            r_max_v = max(n.r for n in mesh.nodes.values())
            z_min_v = min(n.z for n in mesh.nodes.values())
            z_max_v = max(n.z for n in mesh.nodes.values())
            for eid, gps in gp_data.items():
                eta_e = eta_all.get(eid, 0.0)
                for gp in gps:
                    h_e = float(np.sqrt(max(gp['area'], 0.0)))
                    _pending_gp_features.append({
                        'r': gp['r'], 'z': gp['z'],
                        'sigma_rr': gp['sigma_rr'], 'sigma_zz': gp['sigma_zz'],
                        'sigma_rz': gp['sigma_rz'], 'sigma_tt': gp['sigma_tt'],
                        'eta_e': eta_e, 'h_e': h_e,
                        'cycle': float(cycle),
                        'nu': nu_val, 'mu': mu_val, 'p': p_val,
                        'r_min': r_min_v, 'r_max': r_max_v,
                        'z_min': z_min_v, 'z_max': z_max_v,
                    })

        # --- Neural refinement bypass: predict after cycle 0, skip remaining ---
        if _neural_predictor is not None and cycle == 0:
            corrections = _neural_predictor.predict_node_stress_corrections(
                mesh, ibem_strategy.last_eta_all, cycle, config
            )
            for nid, delta in corrections.items():
                mesh.nodes[nid].neural_stress_correction = delta
            print(f"[NeuralRefinement] Applied corrections to {len(corrections)} nodes; skipping remaining cycles.")
            break
    ...
    # --- Dataset collection: resolve targets from final refined mesh ---
    if getattr(config, 'collect_dataset', False) and _pending_gp_features:
        _dataset_rows = _resolve_dataset_targets(mesh, _pending_gp_features)
        if _dataset_rows:
            from .dataset_generator import save_rows_to_hdf5
            out_path = getattr(config, 'dataset_output_path', 'dataset.h5')
            save_rows_to_hdf5(_dataset_rows, out_path, append=True)
            print(f"[Dataset] Saved {len(_dataset_rows)} rows → {out_path}")

    return shape_func, mesh, cycle_results


def _resolve_dataset_targets(final_mesh, pending_features: list) -> list:
    """For each pending coarse GP, query refined stress at same (r,z) and build full row."""
    from .postprocessors.fem_postprocessor import _recover_raw
    from .postprocessors.fem_sampling import find_isoparametric_coords
    import numpy as np

    nodal_stress = _recover_raw(final_mesh)

    rows = []
    for feat in pending_features:
        r_q, z_q = feat['r'], feat['z']
        found = False
        for elem in final_mesh.elements.values():
            try:
                xi, eta_iso = find_isoparametric_coords(elem, final_mesh, r_q, z_q)
            except ValueError:
                continue
            N_vals, _, _ = elem.shape_func.evaluate(xi, eta_iso)
            N_vals = np.asarray(N_vals, dtype=float).reshape(-1)
            srr = szz = srz = stt = 0.0
            for a, nid in enumerate(elem.node_ids):
                w = float(N_vals[a])
                s = nodal_stress[nid]
                srr += w * s[0]; szz += w * s[1]; srz += w * s[2]; stt += w * s[3]
            rows.append({**feat,
                         'sigma_rr_ref': srr, 'sigma_zz_ref': szz,
                         'sigma_rz_ref': srz, 'sigma_tt_ref': stt})
            found = True
            break
        # Skip GPs that fall outside the final mesh (e.g. due to numerical tolerance)
    return rows
